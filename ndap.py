...
class NDArrayPlotter(object):
    ...
    def set_face_colors(self,color):
        shape=list(self.array_.shape)
        shape.append(6)
        self.face_colors=np.zeros(shape,dtype=('a10'))
    ...
    def render(self, array=None, text=None, azim=-15, elev=15,ax=None,faces=False,labels=True,space=0.5):
        ...
        for l in range(0, array.shape[0]):
            for m in range(0, array.shape[1]):
                for n in range(0, array.shape[2]):
                    ...
                    alpha = self.alphas[l, n, m]
                    if faces:
                        color = self.face_colors[l, m, n]
                    else:
                        color=self.colors[l,n,m]
        ...
        for size in array.shape:
            if size > highest:
                highest = size
        ax.set_xlim((-1*space,highest+space))
        ax.set_ylim((-1*space,highest+space))
        ax.set_zlim((-1*space,highest+space))
        ...
        axis_label_format = r"\textbf{\emph{%s}}"

        laxis_label = axis_label_format % "x" 
        maxis_label = axis_label_format % "z" 
        naxis_label = axis_label_format % "y" 

        #(M_scl*(M+M_skew))+0.5
        if labels:
            ax.text(
                (L_scl*(L+L_skew))/2.0, -0.5, 0,
                laxis_label, zdir='x', horizontalalignment='center', verticalalignment='center',fontsize=24
            )
            ax.text(
                0, -0.5, (M_scl*(M+M_skew))/2.0, 
                maxis_label, zdir='z', horizontalalignment='center', verticalalignment='center',fontsize=24
            )
            ax.text(
                0, (N_scl*(N+N_skew))/2.0, -0.5, 
                naxis_label, zdir='y',fontsize=24 
            )
        #plt.axis('off')
...
"""
    ...
    (fig_coord, ax_coors) = plotter.render(text=text_coords,ax=ax)
    (fig_values, ax_value) = plotter.render(text=text_values,ax=ax)
    plt.show()
